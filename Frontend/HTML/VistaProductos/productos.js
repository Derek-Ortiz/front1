...
async function cargarProductos() {
    ...
    try {
        const response = await fetch(`http://localhost:7000/api/negocio/${codigoNegocio}/productos`);
...
async function cargarInsumos() {
    ...
    try {
        const response = await fetch(`http://localhost:7000/api/negocio/${codigoNegocio}/insumosParaProductos`);
        if (!response.ok) {
...
async function openEditModal() {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:7000/api/negocio/${codigoNegocio}/productos/${selectedProductId}`);
        ...
        const insumosResponse = await fetch(`http://localhost:7000/api/negocio/${codigoNegocio}/productos/${selectedProductId}/insumos`);
    ...
    if (imagen) {
        ...
        if (suppliesToAdd.length > 0) {
            for (const supply of suppliesToAdd) {
                await fetch(`http://localhost:7000/api/negocio/${codigoNegocio}/productos/${productoGuardado.id}/insumos`, {
                    method: 'POST',
                    headers: {
...
async function agregarInsumosProducto(productoId, codigoNegocio) {
    try {
        const promises = suppliesToAdd.map(async supply => {
            ...
            const response = await fetch(`http://localhost:7000/api/negocio/${codigoNegocio}/productos/${productoId}/insumos`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
...
async function saveChanges() {
    ...
    console.log("➡ Datos final del producto a enviar:", productoData);
        ...
        if (editSupplies.length > 0) {
            ...
            await fetch(`http://localhost:7000/api/negocio/${codigoNegocio}/productos/${selectedProductId}/insumos`, {
                method: 'DELETE'
            });
            ...
            for (const supply of editSupplies) {
                ...
                const insumoResponse = await fetch(`http://localhost:7000/api/negocio/${codigoNegocio}/productos/${selectedProductId}/insumos`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
        ...
        if (producto.imagen) {
            ...
            if (typeof producto.imagen === 'string') {
                if (producto.imagen.startsWith('data:')) {
                    ...
                    // 
                    imagenUrl = `http://localhost:7000/${producto.imagen}`;
                } else {
                    // Por si acaso viene solo el nombre del archivo
                    imagenUrl = `http://localhost:7000/uploads/${producto.imagen}`;
                }
