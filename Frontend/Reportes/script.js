document.addEventListener('DOMContentLoaded', function() {
    ...
    const API_BASE_URL = 'http://localhost:7000/api/reportes';
    ...
    function inicializar() {
        ...
        if (inputDesde) {
            ...
            desdeFecha = new Date(inputDesde.value);
            console.log('📅 Fecha inicial establecida:', desdeFecha);
        }
    ...
    if (inputDesde) {
        inputDesde.addEventListener('change', function() {
            console.log('📅 Fecha desde cambiada:', inputDesde.value);
            desdeFecha = new Date(inputDesde.value);
            actualizarHastaFecha();
        });
    }
    ...
    function formatearFechaParaAPI(fecha, esInicio = true) {
        if (!fecha) {
            console.error('❌ Fecha no válida para formatear');
            return null;
        }
        ...
        const resultado = esInicio ? 
            `${year}-${month}-${day}T00:00:00` : 
            `${year}-${month}-${day}T23:59:59`;
        
        console.log('📅 Fecha formateada:', { original: fecha, resultado, esInicio });
        return resultado;
    }
    ...
    function actualizarHastaFecha() {
        ...
        if (hastaDiv) {
            hastaDiv.textContent = formatFecha(hasta);
        }
        
        // Actualizar los labels del gráfico con las nuevas fechas
        if (salesChart) {
            salesChart.data.labels = generarLabelsPorTipo(tipoFiltro, desdeFecha, hastaFecha);
            salesChart.update();
        }
        
        console.log('📅 Fecha hasta actualizada:', hastaFecha);
        console.log('📋 Labels del gráfico actualizados:', salesChart?.data?.labels);
    }
    ...
    const fechaDesde = desdeFecha.toISOString().split('T')[0];
    const fechaHasta = hastaFecha.toISOString().split('T')[0];
       const url = `${API_BASE_URL}/${ID_NEGOCIO}/exportar?desde=${fechaDesde}&hasta=${fechaHasta}`;
    
    console.log('📄 Descargando reporte:', url);
    
    // Simplemente abrir la URL - el navegador descargará automáticamente
